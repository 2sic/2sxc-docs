...
```
The `@foreach` will go through all the items. Each item is of the type [IEntity](xref:NetCode.DynamicCode.Entity). To make it easier to template, we convert it to a [Dynamic Entity](xref:NetCode.DynamicCode.DynamicEntity) using `AsDynamic` and then we can just show the name with `.FullName`. 
